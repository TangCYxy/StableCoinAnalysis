...
contract UCoinCommentTransferIntegrated {
    // bytes4(keccak256("transfer(address,uint256)"))
    bytes4 public constant selectorTransfer = 0xa9059cbb;
    ...
    // 标准，写入存储并扔出event
    function transferFromWithMsg(address coinContract, address from, address recipient, uint256 amount, string memory comment, bytes32 sigR, bytes32 sigS, uint8 sigV)
    external
    returns (bool)
    {
        // do transfer through out ucoin.executeMetaTransaction()
        bytes memory functionSignature = abi.encodeWithSelector(selectorTransfer, recipient, amount);
        INativeMetaTransaction(coinContract).executeMetaTransaction(from, functionSignature, sigR, sigS, sigV);
        // do msg
